...
/* Hosts configuration */
$hosts = !empty($_ENV['PMA_HOST']) ? [$_ENV['PMA_HOST']] : ['db'];
$verbose = !empty($_ENV['PMA_VERBOSE']) ? [$_ENV['PMA_VERBOSE']] : [];
$ports = !empty($_ENV['PMA_PORT']) ? [$_ENV['PMA_PORT']] : [];

/* Multiple Hosts */
if (!empty($_ENV['PMA_HOSTS'])) {
...
/* Server settings with enforced cookie auth */
for ($i = 1; isset($hosts[$i - 1]); $i++) {
    $cfg['Servers'][$i]['host'] = $hosts[$i - 1];
    $cfg['Servers'][$i]['auth_type'] = 'cookie'; // Force connection request
    ...
    $cfg['Servers'][$i]['AllowNoPassword'] = false; // Disable access without password
}
